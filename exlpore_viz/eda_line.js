...
const margin = { top: 60, right: 40, bottom: 80, left: 80 }; // Increased left margin for y-axis label
...
d3.csv("../data/df.csv")
  .then((raw) => {
    raw.forEach((d) => {
      ...
      d.trial16 = d.cond === "Calming" ? global : global - 16;
    });
    ...
    svg
      ...
      .attr("y", svg.attr("height") - 40) // Moved up from -20 to -40
    ...
    svg
      ...
      .attr("y", 20) // Position near left edge
      .attr("text-anchor", "middle")
      .attr("font-size", "14px")
      .attr("font-weight", "600")
      .attr("fill", "#333")
    ...
    // 10) Legend (moved down and aligned with x-axis label)
    const legend = svg
      .append("g")
      ...
      .attr("transform", (d, i) => {
        ...
        return `translate(${x0 + i * spacing}, ${svg.attr("height") - 15})`; // Aligned with x-axis label level
      });
    ...
    // 12) Draw & animate loop
    function animate() {
      const paths = g.selectAll("path.eda").data(series, (d) => d.name);
